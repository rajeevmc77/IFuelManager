/******************************************************************************
* Reference sketch  vehicle telematics data feed Freematics ONE+
* Author - Akshara Gireesh Murali
* Description - Code to be deployed in Freematics One + ESP32 microcontroller  
* to pull Car OBD data and to send the same to IFleetManager Server over HTTP POST
* Date - 15-03-2019
******************************************************************************/
...
PID_POLLING_INFO obdData[]= {
  ...
  // {PID_COOLANT_TEMP, 3,"CoolantTemp"},
  // {PID_INTAKE_TEMP, 3,"IntakeTemp"},
};
...
void processOBD()
{
  memset(obdDataBuffer,'0',sizeof(obdDataBuffer));
  char obdValBuff[100] = {0};
...
char formatedOBDBuff[512] = {0};
char * getFormatedOBDData(){  
  int n = 0, size = sizeof(formatedOBDBuff);
  memset(formatedOBDBuff,0,size);
  for (int i = 0; i < sizeof(obdData) / sizeof(obdData[0]); i++) {
    char tempBuff[128] ={0};
...
void process()
{
  ...
  if (millis() - teleClient.lastSentTime >= sendingInterval && cache.samples() > 0) {
    ...
    // create new buffer to store JSON data 
