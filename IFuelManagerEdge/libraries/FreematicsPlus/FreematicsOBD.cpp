...
int COBD::readFuelLevel(){

		char buffer[64]="2201\r";
			delay(10);
			if (!link->sendCommand(buffer, buffer, sizeof(buffer), OBD_TIMEOUT_SHORT) || !strstr(buffer, "OK")) {
				Serial.print("Failed to Obtain Fuel Level - Honda  ");
				Serial.println(buffer);
			}	
			else {
				Serial.print("Honda Fuel Level");
				Serial.println(buffer);
			}	
			return 1;
}
...
bool COBD::readPID(byte pid, int& result)
{
	...
	link->send(buffer);
	delay(100);
	int ret = link->receive(buffer, sizeof(buffer), OBD_TIMEOUT_LONG);
...
bool COBD::init(OBD_PROTOCOLS protocol)
{
	...
	for (byte n = 0; n < 3; n++) {
		...
		stage = 2;
		delay(10);
		// Removed By Rajeev M C for testing purpose alone.
